...
1. **使用已构建好的安装包（推荐）**：仓库已把 `output/` 下的 4 个文件打包为 **`VoxCPM2_TTS_v5.0_Setup.zip`**（位于本仓库根目录，与源码一同 clone 得到），解压后双击 `VoxCPM2_TTS_v5.0_Setup.exe` 即可安装。
...
双击桌面上的 **VoxCPM2 TTS 中文版 - 网页界面** 图标，浏览器会自动打开 http://127.0.0.1:8000 的图形化界面（若 8000 被占用会自动顺延到下一个可用端口）。
...
随后自动打开浏览器 http://127.0.0.1:8000 （若 8000 被占用会自动顺延到下一个可用端口）。Web UI 支持麦克风录制参考音频、自定义音色描述、可编辑模型/输出目录，并会在路径面板显示「降噪模型：已内置（离线可用）」。
