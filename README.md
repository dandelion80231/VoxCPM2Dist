...
- **v5.2.1（当前）**：**修复安装器解压卡死（致命）**——原安装器自带的 7za 为 32 位，处理 >4GB 的模型权重 `model.safetensors`（约 4.6GB）时在文件写完后收尾阶段永久挂起，导致安装进度条几乎不动、数小时不结束（此前多次"卡死"均因此，并非进度条显示问题）。已替换为 **64 位 7za + 7za.dll/7zxa.dll**，并加装解压看门狗（45 分钟总上限 / 5 分钟无进度判卡死）与失败可读提示；`build_installer.ps1` 改为强制 x64 防止回归。安装包文件名仍为 v5.2（无需重打安装包本体）。继承 v5.2 全部功能。
- **v5.2**：Apple 风格 Web UI 重做；安装向导新增「创建桌面快捷方式」与「立即运行」复选框；启动器 Banner 修复 CJK 中文双宽对齐；继承 v5.0.2 全部功能。
