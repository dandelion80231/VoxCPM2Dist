...
# tensorboardX 是「可选」依赖：离线分发包默认不含它（仅用于训练可视化，不影响训练本身）。
# 缺失时降级为无日志模式——所有 writer.* 调用均已用 `if writer:` / `if writer is not None` 守卫。
try:
...
# ---------------------------------------------------------------------------
# [兼容补丁] datasets 5.x 把音频解码强制绑到 torchcodec（需 ffmpeg，离线包没有），
# 且返回 AudioDecoder 对象；而 voxcpm 2.0.3 的 data.py 按旧版 dict 接口访问
# audio["array"] / audio["sampling_rate"]（见 voxcpm/training/data.py:143-144,150）。
# 这里用已内置的 soundfile 恢复旧接口，让 compute_sample_lengths 与训练采样都能离线跑。
# ---------------------------------------------------------------------------
import io as _io
...
def _audio_decode_soundfile(self, value, token_per_repo_id=None):
    if not self.decode:
        raise RuntimeError("Decoding is disabled for this feature. Please use Audio(decode=True) instead.")
    path = value.get("path")
    data_bytes = value.get("bytes")
    if path is None and data_bytes is None:
        raise ValueError(f"An audio sample should have one of 'path' or 'bytes' but both are None in {value}.")
    if data_bytes is not None:
        data, sr = _sf.read(_io.BytesIO(data_bytes), always_2d=False)
    else:
        data, sr = _sf.read(path, always_2d=False)
    if data.ndim > 1:
        data = data.mean(axis=1)
    return {"array": _np.asarray(data, dtype=_np.float32), "sampling_rate": int(sr)}


_AudioFeature.decode_example = _audio_decode_soundfile
...
def train(
    ...
    writer = SummaryWriter(log_dir=str(tb_dir)) if (accelerator.rank == 0 and SummaryWriter is not None) else None
            ...
            with torch.no_grad():
                with autocast_ctx:
                    generated = unwrapped_model.generate(
                        text=text, inference_timesteps=10, cfg_value=2.0, seed=42
                    )
...
if __name__ == "__main__":
    ...
    if config_file:
        ...
        # 相对路径一律按「yaml 所在目录」解析（配置注释约定），
        # 而非按进程 CWD —— 否则从 app/Scripts 启动时 ../../model 等会指错目录直接崩。
        _cfg_dir = Path(config_file).resolve().parent
        for _k in ("pretrained_path", "train_manifest", "val_manifest", "save_path", "tensorboard"):
            _v = yaml_args.get(_k)
            if _v:
                _p = Path(str(_v))
                if not _p.is_absolute():
                    yaml_args[_k] = str(_cfg_dir / _p)
        train(**yaml_args)
    else:
