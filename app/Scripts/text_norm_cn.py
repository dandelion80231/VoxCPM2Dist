...
_USER_RULES_FILE = os.path.join(
    os.path.dirname(os.path.abspath(__file__)), "num_norm_extra.txt"
)
...
_MEASURE_ALT = "|".join(
    re.escape(u) for u, _ in sorted(_MEASURE_UNITS, key=lambda x: -len(x[0]))
)
...
def normalize_text(text: str) -> str:
    ...
    text = re.sub(
        r"(\d{4})年(\d{1,2})月",
        lambda m: (
            f"{_num_to_ch(m.group(1))}年{_int_to_chinese_small(_safe_int(m.group(2)))}月"
        ),
        text,
    )
    ...
    text = re.sub(
        r"(\d+)年\s*[-—~～]\s*(\d+)年",
        lambda m: f"{_num_to_ch(m.group(1))}年到{_num_to_ch(m.group(2))}年",
        text,
    )
    ...
    text = re.sub(
        r"(\d+)(?=" + _HOTLINE_SUFFIX + r")", lambda m: _hotline_to_ch(m.group(1)), text
    )
    # 1.6) 电话号码：逐位读，1 读成「幺」
    ...
    text = re.sub(
        ...
        lambda m: (
            m.group(1) + m.group(2) + _hotline_to_ch(re.sub(r"[^\d]", "", m.group(3)))
        ),
        text,
    )
    ...
    text = re.sub(
        r"(?<!\d)(1[3-9](?:[- ]?\d){9})(?!\d)",
        lambda m: _hotline_to_ch(re.sub(r"[^\d]", "", m.group(1))),
        text,
    )
    ...
    def time_repl(m):
        ...
        if s is not None:
            ss = _safe_int(s)
            out += (
                _num_to_ch(f"0{ss}") if ss < 10 else _int_to_chinese_small(ss)
            ) + "秒"
        return out
    ...
    text = re.sub(
        r"(\d{1,3})\s*[:：]\s*(\d{1,3})",
        lambda m: (
            f"{_num_to_chinese_value(_safe_int(m.group(1)))}比{_num_to_chinese_value(_safe_int(m.group(2)))}"
        ),
        text,
    )
    ...
    text = re.sub(
        ...
        lambda m: (
            ("负" if m.group(1) else "")
            + f"{_num_to_chinese_value(_safe_int(m.group(3)))}分之{_num_to_chinese_value(_safe_int(m.group(2)))}"
        ),
        text,
    )
    ...
    text = re.sub(
        r"(?<!\d年)(\d{1,3})\s*[-~～]\s*(\d{1,3})(?!年)",
        lambda m: (
            f"{_num_to_chinese_value(_safe_int(m.group(1)))}到{_num_to_chinese_value(_safe_int(m.group(2)))}"
        ),
        text,
    )
    ...
    def pct_repl(m):
        ...
        core = (
            f"百分之{_decimal_to_ch(s)}"
            if "." in s
            else f"百分之{_num_to_chinese_value(_safe_int(s))}"
        )
    ...
    text = re.sub(
        r"([A-Za-z]+)[-_](\d+(?:\.\d+)?)",
        lambda m: f"{m.group(1)}{_num_or_decimal(m.group(2))}",
        text,
    )
    ...
    text = re.sub(
        r"(?<![A-Za-z\u00a7])([A-Z]{2,})(?![a-z])",
        lambda m: (
            " ".join(m.group(1)) if m.group(1) not in _ACRONYM_AS_WORD else m.group(1)
        ),
        text,
    )
