...
## 2.5 数字 / 文本归一化（text_norm_cn.py）：规则与两个白名单

`app/Scripts/text_norm_cn.py` 纯正则（仅依赖 `re`、零外部依赖）把阿拉伯数字、版本号、百分号、日期、单位等转成自然中文读法，并被网页端 `vox_web_ui.py` 与 CLI `voxcpm_tts_v5_longtext.py` 共用（同目录 `from text_norm_cn import normalize_text`）。**只此一份**，改动无需同步两边。

### 已覆盖的读法规则
- 日期（`2024年1月15日` / `2024-01-15`）、年份区间（`2024年-2025年`）、裸年份逐位（`2025年`→`二零二五年`）；
- 热线（`110`→`妖妖灵`，`1`→`幺`）、电话（手机号 11 位逐位、`1`→`幺`）、温度（`-10°C`→`零下十摄氏度`）；
- 时间（`14:30`→`十四点三十分`）、比分（`3:5`→`三比五`）、分数（`3/4`→`四分之三`）、范围（`10~20`→`十到二十`）；
- 金额（`¥100`→`一百元`、`$10.5`→`十点五美元`）、百分数（`67%`→`百分之六十七`，含 `±`/`-`）、计量单位（`km`→`公里`、`kg`→`千克`、`25km/h`→`二十五公里每小时`）；
- 数学符号（`+×÷=`）、普通小数（`5.6`→`五点六`）、版本号（`3.12.10`→`三点一二点一零`）；
- 普通整数 **≤12 位按位值读**（`2亿`→`两亿元`、支持到亿/兆），13 位及以上逐位（身份证 18 位等）；
- **二/两归一化**（`二百`→`两百`、`二亿`→`两亿`，仅最高位为 2）；**全角→半角**（`０-９`/`Ａ-Ｚ`/`ａ-ｚ` 及全角拉丁标点转半角；中文逗号/句号保留以保 TTS 停顿）；**编号关键词守卫**（见下）。

### 白名单 ①：编号关键词守卫（数字逐位读的 whitelist）
数字**紧跟**以下关键词时强制逐位读，避免 9–12 位订单号被当成数量级：
`订单号 / 快递单号 / 手机号码 / 手机号 / 编号 / 账号 / 卡号 / 密码 / 手机 / 微信号 / QQ / 单号 / 工号 / 学号 / 证号 / 批号 / 货号`
（复刻 WeTextProcessing 的 whitelist 思路，长词前置避免被短词截断漏接数字。）
**扩展方法**：在 `normalize_text()` 内的 `_ID_KEYWORDS` 正则交替里加关键词即可（注意**长词放前面**，如 `手机号` 在 `手机` 前，否则会被短词先匹配而漏接数字）。

### 白名单 ②：英文缩写「按单词读」白名单（`_ACRONYM_AS_WORD`）
连续大写字母默认会被 Z 规则插空格、交给 G2P **逐字母读**（`CPU`→`C P U`、`USB`→`U S B`、`GPT`→`G P T`）。但机构/品牌名应按**单词**读（`NASA`→`['næsə]`、`Intel`、`Google`…），故用 `_ACRONYM_AS_WORD` 集合豁免。
**收录原则**：只收「按单词读」的 acronym（`NASA/NATO/UNESCO/UNICEF/OPEC/ASEAN/APEC/WHO/CERN/FIFA/INTERPOL/NAFTA`、`NOAA/ISO`、`COVID/SARS/AIDS`，以及 `Intel/Nvidia/Sony/Samsung/Google/Apple/Tesla/Microsoft/Huawei/…` 等品牌）；**刻意排除**逐字母读的 initialism（`FBI/CIA/BBC/IBM/UN/UK/EU/WTO/IMF/GPS/CEO/USB/CPU/AMD/HP/BMW/LG…`），误收会把它们读成单词。
**扩展方法**：直接往 `_ACRONYM_AS_WORD` 集合里加**全大写**词即可。⚠️ 品牌名常以全大写出现（如 `GOOGLE`/`INTEL`），若不加入集合，Z 规则会把它拆成单字母 `G O O G L E`，所以**全大写品牌形式也必须加**。
